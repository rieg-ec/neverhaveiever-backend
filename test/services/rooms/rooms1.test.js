...
if (!RoomsConnections.getRoomConnection(roomID).users[0].status.isAdmin) {
  throw Error('user creating room must be admin');
}
...
if (RoomsConnections.getRoomConnection(roomID) !== RoomsConnections.connections[roomID]) {
  ...
  throw Error(`getRoomConnection returns ${returns}, should return ${RoomsConnections.connections[roomID]}`);
}

if (RoomsConnections.getRoomConnection('12345') !== null) {
  const returns = RoomsConnections.getRoomConnection('12345');
  throw Error(`getRoomConnection returns ${returns}, should return ${null}`);
}
/* ********************************************************* */
...
if (RoomsConnections.getConnectedUsers(roomID)
  ...
  const returns = RoomsConnections.getConnectedUsers(roomID);
  throw Error(`getConnectedUsers with namesOnly false returns ${returns},
...
if (JSON.stringify(RoomsConnections.getConnectedUsers(roomID, true))
  ...
  throw Error(`getConnectedUsers with namesOnly true returns ${returns}, should return ${JSON.stringify(['Ramon', 'Juan', 'Pedro', 'Maria'])}`);
}
/* ********************************************************* */
...
if (RoomsConnections.getRoomConnection(newRoomID).users[0].username !== 'Ramon') {
  throw Error('createRoom not adding user Ramon to users');
}
/* ********************************************************* */
...
if (!RoomsConnections.getConnectedUsers(roomID, true).includes('Panchito')) {
  throw Error(`joinRoom not joining ${testUser5.username} to room ${roomID}`);
}

try {
  RoomsConnections.joinRoom(testUser5.username, testUser5.userID, 'asdda');
  throw Error('joinRom adding users to non existing room');
} catch {}
...
const { user, roomID: removedRoomID } = RoomsConnections
  .removeUserFromRoom(testUser5.userID);

if (removedRoomID !== testRoom.roomID) {
  throw Error(`rooms id dont match ${removedRoomID} ${testRoom.roomID}`);
}

if (user.username !== testUser5.username) {
  throw Error(`usernames dont match ${user.username} ${testUser5.username}`);
}

if (RoomsConnections.getConnectedUsers(testRoom.roomID, true).includes(testUser5.username)) {
  throw Error(`removeUserFromRoom not removing ${testUser5.userID} from room`);
}
/* ********************************************************* */
...
if (!RoomsConnections.getRoomConnection(testRoom.roomID).status.inGame) {
  throw Error(`startGame doesn't set inGame
...
if (JSON.stringify(RoomsConnections.getRoomConnection(testRoom.roomID).status.usersIDVotedReady)
    !== JSON.stringify([])) {
  throw Error('startGame doesn\'t set usersIDVotedReady to []');
}
/* ********************************************************* */
...
if (!RoomsConnections.getRoomConnection(testRoom.roomID).room.currentStatement
    === statement) {
  throw Error(`newStatement doesn't set currentStatement to ${statement},
              instead to ${RoomsConnections.getRoomConnection(testRoom.roomID).room.currentStatement}`);
}

if (!RoomsConnections.getRoomConnection(testRoom.roomID).room.statements
  .includes(statement)) {
  throw Error(`newStatement doesn't push ${statement} to statements:
    ${RoomsConnections.getRoomConnection(testRoom.roomID).room.statements}`);
}
/* ********************************************************* */
...
if (RoomsConnections.getRoomConnection(testRoom.roomID).status.usersIDVotedReady.length !== 1) {
  throw Error('voteUser not adding user to usersIDVotedReady');
}

RoomsConnections.voteUser(testUser1.username, testUser2.userID, testRoom.roomID);
if (RoomsConnections.getRoomConnection(testRoom.roomID).status.usersIDVotedReady.length !== 1) {
  throw Error('voteUser not handling user voting twice');
}

if (!RoomsConnections.getRoomConnection(testRoom.roomID).status.votedUsers
  .includes(testUser1.username)) {
  throw Error(`voteUser doesnt add ${testUser1.username} to votedUsers`);
}

RoomsConnections.getRoomConnection(testRoom.roomID).users.forEach((u) => {
  RoomsConnections.voteUser(testUser1.username, u.userID, testRoom.roomID);
});
/* ********************************************************* */
...
if (votes[testUser2.username] !== 3 || votes[testUser3.username] !== 1) {
  throw Error(`getVotes error ${votes[testUser2.username]}`);
}
...
if (!RoomsConnections.getRoomConnection(testRoom.roomID).status.inSummary) {
  throw Error('setUserReady not setting room status to inSummary: true');
}
...
if (RoomsConnections.getRoomConnection(testRoom.roomID).status.usersIDReady.length
  ...
  throw Error(`${RoomsConnections.getRoomConnection(testRoom.roomID).status.usersIDReady.length}
              !== ${RoomsConnections.getRoomConnection(testRoom.roomID).users.length}`);
}

if (!RoomsConnections.getRoomConnection(testRoom.roomID).status.usersIDReady
  .includes(testUser1.userID)) {
  throw Error('setUserReady not adding userID to usersReady');
}

RoomsConnections.setUserReady(testUser1.userID, testRoom.roomID);
if (RoomsConnections.getRoomConnection(testRoom.roomID).status.usersIDReady.length !== 1) {
  throw Error('setUserReady adding user twice to usersReady');
}

RoomsConnections.getRoomConnection(testRoom.roomID).users.forEach((u) => {
  RoomsConnections.setUserReady(u.userID, testRoom.roomID);
});
/* ********************************************************* */
